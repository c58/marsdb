...
describe('IndexManager', () => {
  ...
  it('should be created with _id index', () => {
    idxMan.indexes['_id'].should.be.instanceof(CollectionIndex);
  });
  ...
  describe('#buildIndex', function () {
    it('should build an existing index', function () {
      return Promise.all([
        ...
        idxMan.indexes._id.getMatching(5).should.be.deep.equal([5]);
        idxMan.indexes._id.reset();
        idxMan.indexes._id.getMatching(1).should.be.deep.equal([]);
        idxMan.indexes._id.getMatching(5).should.be.deep.equal([]);
  ...
  describe('#buildAllIndexes', function () {
    it('should build all existing indexes', function () {
      return Promise.all([
        ...
        idxMan.indexes.b.getMatching(2).should.be.deep.equal([]);
        idxMan.indexes.b.getMatching(6).should.be.deep.equal([]);
        idxMan.indexes._id.getMatching(1).should.be.deep.equal([]);
        idxMan.indexes._id.getMatching(5).should.be.deep.equal([]);
  ...
  describe('#createIndex', function () {
    it('should create and build index when index does not exists', () => {
      return Promise.all([
        ...
        return idxMan.createIndex('b');
      }).then(() => {
        idxMan.indexes['b'].should.be.instanceof(CollectionIndex);
        idxMan.indexes.b.getMatching(2).should.be.deep.equal([1]);
        idxMan.indexes.b.getMatching(6).should.be.deep.equal([5]);
      });
    });
    ...
    it('should return promise for already running index building task', () => {
      return Promise.all([
        ...
        const runProm = idxMan.createIndex('_id', {forceRebuild: true});
        const anotherRunProm = idxMan.createIndex('_id', {forceRebuild: true});
    ...
    it('should be able to force rebuild indexes', () => {
      return Promise.all([
        ...
        idxMan.indexes._id.getMatching(1).should.be.deep.equal([]);
        idxMan.indexes._id.getMatching(5).should.be.deep.equal([]);
        return idxMan.createIndex('_id', {forceRebuild: true});
      }).then(() => {
        idxMan.indexes._id.getMatching(1).should.be.deep.equal([1]);
        idxMan.indexes._id.getMatching(5).should.be.deep.equal([5]);
      });
  ...
  describe('#indexDocument', function () {
    it('should index document in all existing indexes', function () {
      return Promise.all([
        idxMan.createIndex('b', {unique: true}),
        idxMan.createIndex('a')
      ]).then(() => {
