...
LocalCollection._compileProjection = function (fields) {
  ...
  var transform = (doc, ruleTree) => {
    ...
    _each(ruleTree, (rule, key) => {
      if (!Object.hasOwnProperty(doc, key)) {
        return;
      }
      if (_check.object(rule)) {
        // For sub-objects/subsets we branch
        if (_check.object(doc[key])) {
  ...
  return (obj) => {
    ...
    if (_idProjection && Object.hasOwnProperty(obj, '_id')) {
      res._id = obj._id;
    }
    if (!_idProjection && Object.hasOwnProperty(res, '_id')) {
      delete res._id;
    }
    return res;
  };
...
pathsToTree = function (paths, newLeafFn, conflictFn, tree) {
  ...
  _each(paths, (keyPath) => {
    ...
    var success = _every(pathArr.slice(0, -1), (key, idx) => {
      if (!Object.hasOwnProperty(treePos, key)) {
        treePos[key] = {};
    ...
    if (success) {
      ...
      if (!Object.hasOwnProperty(treePos, lastKey)) {
