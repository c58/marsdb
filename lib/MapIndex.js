...
export class MapIndex extends CollectionIndex {
  ...
  insert(doc) {
    ...
    if (doc._id && valueKey) {
        ...
        || typeof valueKey === 'string')
      {
        ...
        throw new Error(`CollectionIndex(...): ${typeof valueKey} type of index
          is not supported yet. It can be String, Number, Date.`);
      }
    }
    return false;
  }
  ...
  update(oldDoc, newDoc) {
    invariant(
      ...
      `CollectionIndex(...): old and new documents must have the same "_id" when doing update`
    );
