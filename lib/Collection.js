...
// Track first stack execution for printing
// warning of lazy initialization in first stack
let _firstStackExecuted = false;
setTimeout(() => _firstStackExecuted = true, 0);
...
export class Collection extends EventEmitter {
  constructor(name, options = {}) {
    ...
    this._writeQueue = new PromiseQueue(1);

    // Shorthand for defining in-memory collection
    if (options.inMemory) {
      options.cursorClass = CursorObservable;
      options.delegate = CollectionDelegate;
      options.storageManager = StorageManager;
      options.indexManager = IndexManager;
      options.idGenerator = ShortIdGenerator;
    }
  }
  ...
  _lazyInitCollection() {
    if (!this._initialized) {
      if (!_firstStackExecuted) {
        console.warn(`You are using "${this.modelName}" collection in the` +
          `same execution cycle when it was created. It is not recommended` +
          `because some Collection modifier may be executed after you create` +
          `the collection. Please, use collection in next execution cycle` +
          `(by process.nextTick(fn) or setTimeout(fn, 0))`);
      }
